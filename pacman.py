...
def readCommand(argv):
    ...
    tmp = []
    if (options.ghost == "RandomGhost" or options.ghost == "DirectionalGhost"):
        ghosts = loadAgent(options.ghost, noKeyboard)
        for i in range(0, options.numGhosts):
            tmp.append(ghosts(i+1))
    else:
        ghosts1 = loadAgent("RandomGhost", noKeyboard)
        ghosts2 = loadAgent("DirectionalGhost", noKeyboard)
        num_directional = int(0.4*args["layout"].getNumGhosts())
        num_random = args["layout"].getNumGhosts() - num_directional
        for i in range(0, args["layout"].getNumGhosts()):
            if (i < num_directional):
                tmp.append(ghosts2(i+1))
            else:
                tmp.append(ghosts1(i+1))
    args['ghosts'] = tmp
