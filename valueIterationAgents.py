...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        "*** YOUR CODE HERE ***"
        for iter_count in range(0, self.iterations):
            new_values = {}
            for state in self.mdp.getStates():
                for action in self.mdp.getPossibleActions(state):
                    value = 0
                    for transition_info in self.mdp.getTransitionStatesAndProbs(state, action):
                        next_state = transition_info[0]
                        transition_prob = transition_info[1]
                        reward = self.mdp.getReward(state, action, next_state)
                        value += transition_prob*(reward + self.discount*self.values[next_state])
                    if (state not in new_values or new_values[state] < value):
